
<template>
  <main class="site-main about grey-background">

			<section class="page-header">
				<h1 class="text-center text-white text-uppercase fw-700 mb-0">معلومات عنا</h1>
				<div class="bc-container flex align-items-center">
					<svg version="1.1" class="bc-left" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 45 60" style="enable-background:new 0 0 45 60;" xml:space="preserve">
						<polygon style="fill:#E11D28" points="35,0 15,60 25,60 45,0 "/>
						<polygon style="fill:#FA3E2E" points="20,0 0,60 5,60 25,0 "/>
					</svg>

					<ul class="breadcrumb-nav d-flex align-center justify-center mb-0 list-unstyled">
						<li><a href="#">الرئيسية</a></li>
						<li><a href="#">معلومات عنا</a></li>
					</ul>
					<svg version="1.1" class="bc-right" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 45 60" style="enable-background:new 0 0 45 60;" xml:space="preserve">
						<polygon style="fill:#E11D28" points="20,0 0,60 10,60 30,0 "/>
						<polygon style="fill:#FA3E2E" points="40,0 20,60 25,60 45,0 "/>
					</svg>
				</div>
			</section> <!-- end .page-header -->

			<section id="about" class="top-full bottom-half">
				<div class="container">
					<div class="row">
						<div class="col-12 col-md-6">
							<figure class="about-man">
								<img src="@/assets/img/about-man.png" class="img-fluid" alt="">
							</figure>
						</div> <!-- end .col-12.col-md-6 -->
						<div class="col-12 col-md-6 col-lg-5 offset-lg-1">
							<article class="about-article">
								<h2 class="fw-700 gradient-text text-uppercase"> المشروععنوان  <br> رئيسي عن الصفحة او </h2>
								<p class="text-white ">الذهاب إلى مركز رياضي أو ما يعرف   الذهاب  والإياب يجب أن يكون مناسباً في المصاريف الشهرية و يتوفر به جميع الأدوات والمعدات المطلوبة و نظافة المكان و أن تشعر فيه بالراحة. أما إذا كنت ترغب في ممارسة الرياضة في منزلك وهذا ما يفعله المعظم و طبعاً مزاياه لا رسوم دخول ولاحشود ويمكنك الأنتقال من تمرين إلى آخر دون راحة والعيب هو أنه ليس لديك أحد كي يشجعك أو يوجهك الذهاب إلى مركز رياضي أو ما يعرف   الذهاب  والإياب يجب أن يكون مناسباً في المصاريف الشهرية و يتوفر به جميع الأدوات والمعدات المطلوبة و نظافة المكان و أن تشعر فيه بالراحة. أما إذا كنت ترغب في ممارسة الرياضة في منزلك وهذا ما يفعله المعظم و طبعاً مزاياه لا رسوم دخول ولاحشودز </p>
								<p class="d-none d-sm-block d-md-none"> ويمكنك الأنتقال من تمرين إلى آخر دون راحة والعيب هو أنه ليس لديك أحد كي يشجعك أو يوجهك الذهاب إلى مركز رياضي أو ما يعرف   الذهاب  والإياب يجب أن يكون مناسباً في المصاريف الشهرية و يتوفر به جميع الأدوات والمعدات المطلوبة و نظافة المكان و أن تشعر فيه بالراحة. أما إذا كنت ترغب في ممارسة الرياضة في منزلك وهذا ما يفعله المعظم و طبعاً مزاياه لا رسوم دخول ولاحشود ويمكنك الأنتقال من تمرين إلى آخر دون راحة والعيب هو أنه ليس لديك أحد كي يشجعك أو يوجهك الذهاب إلى مركز رياضي أو ما يعرف   الذهاب  والإياب يجب أن يكون مناسباً في المصاريف الشهرية و يتوفر به جميع الأدوات والمعدات المطلوبة و نظافة المكان و أن تشعر فيه بالراحة. أما إذا كنت ترغب في ممارسة الرياضة في منزلك وهذا ما يفعله المعظم و طبعاً مزاياه لا رسوم دخول ولاحشود ويمكنك الأنتقال من تمرين إلى آخر دون راحة والعيب هو أنه ليس لديك أحد كي يشجعك أو يوجهك  </p>
								
							</article>
						</div> <!-- end .col-12.col-md-6 -->
					</div> <!-- end .row -->
				</div> <!-- end .container -->
			</section> <!-- end #about -->

			<section id="our-trainers" class="top-half bottom-full">
				<div class="container">
					<div class="row">
						<div class="section-heading mb-100 col-12 text-center">
							<p class="gradient-text text-uppercase fw-500 inline-flex align-items-center">
								<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 19 30" style="enable-background:new 0 0 19 30;" xml:space="preserve">
									<g><g><path class="svg-fill-text" d="M14,0v30h5V0H14z M7,30h5V0H7V30z M0,25h5V5H0V25z"/></g></g>
								</svg>
								<span class="ml-15 mr-15 font-weight-bold">نحن ترابيس. هذا هو مانقوم به</span>
								<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 19 30" style="enable-background:new 0 0 19 30;" xml:space="preserve">
									<g><g><path class="svg-fill-text" d="M0,30h5V0H0V30z M7,30h5V0H7V30z M14,5v20h5V5H14z"/></g></g>
								</svg>
							</p>
							<h2 class="text-white fw-700 text-uppercase mb-25">عن المدرب او صاحب المشروع</h2>
							<p class="mb-0 text-white" >الذهاب إلى مركز رياضي أو ما يعرف <br>  الذهاب  والإياب يجب أن يكون مناسباً في المصاريف الشهرية </p>
						</div> <!-- end .col-12 -->
					</div> <!-- end .row -->
                       <template >
						  <div class="trainer-slider">  
							<VueSlickCarousel    v-bind="trainerslider" ref="carousel">				
									<div class="row trainer-bio d-flex align-items-center">
										<div class="col-12 col-md-6 col-lg-6">
											<h3 class="text-white text-uppercase fw-700 text-white mb-30">محمد ابو شماله</h3>
											<p class="trainer-body">الذهاب إلى مركز رياضي أو ما يعرف   الذهاب  والإياب يجب أن يكون مناسباً في المصاريف الشهرية و يتوفر به جميع الأدوات والمعدات المطلوبة و نظافة المكان و أن تشعر فيه بالراحة. أما إذا كنت ترغب في ممارسة الرياضة في منزلك وهذا ما يفعله المعظم و طبعاً مزاياه لا رسوم دخول ولاحشود ويمكنك الأنتقال من تمرين إلى آخر دون راحة والعيب هو أنه ليس لديك أحد كي يشجعك أو يوجهك الذهاب إلى مركز رياضي أو ما يعرف   الذهاب  والإياب يجب أن يكون مناسباً في المصاريف الشهرية و يتوفر به جميع الأدوات والمعدات المطلوبة و نظافة المكان و أن تشعر فيه بالراحة. أما إذا كنت ترغب في ممارسة الرياضة في منزلك وهذا ما يفعله المعظم و طبعاً مزاياه لا رسوم دخول ولاحشودز ويمكنك الأنتقال من تمرين إلى آخر دون راحة والعيب هو أنه ليس لديك أحد كي يشجعك أو يوجهك الذهاب إلى مركز رياضي أو ما يعرف   الذهاب  والإياب يجب أن يكون مناسباً في المصاريف الشهرية و يتوفر به جميع الأدوات والمعدات المطلوبة و نظافة المكان و أن تشعر فيه بالراحة. أما إذا كنت ترغب في ممارسة الرياضة في منزلك وهذا ما يفعله المعظم و طبعاً مزاياه لا رسوم دخول ولاحشود ويمكنك الأنتقال من تمرين إلى آخر دون راحة والعيب هو أنه ليس لديك أحد كي يشجعك أو يوجهك الذهاب إلى مركز رياضي أو ما يعرف   الذهاب  والإياب يجب أن يكون مناسباً في المصاريف الشهرية و يتوفر به جميع الأدوات والمعدات المطلوبة و نظافة المكان و أن تشعر فيه بالراحة. أما إذا كنت ترغب في ممارسة الرياضة في منزلك وهذا ما يفعله المعظم و طبعاً مزاياه لا رسوم دخول ولاحشود ويمكنك الأنتقال من تمرين إلى آخر دون راحة والعيب هو أنه ليس لديك أحد كي يشجعك أو يوجهك  </p>
											 <div class="buttons-group flex align-items-center mt-30">
												<a href="#" class="btn btn-bottom-border mr-10">Facebook</a>
												<a href="#" class="btn btn-bottom-border mr-10">Instagram</a>
												<a href="#" class="btn btn-bottom-border">Linkedin</a>
											</div>
										</div> <!-- end .col-12 col-lg-5 lg-push-1 -->

										<div class="col-12 col-md-6 col-lg-5  offset-lg-1">
											<img src="@/assets/img/about/layer-5.png" class="img-fluid trainer-img" alt="">
										</div>
									</div> <!-- end #trainer-bio -->
									<div class="row trainer-bio d-flex align-items-center">
										<div class="col-12 col-md-6 col-lg-6">
											<h3 class="text-white text-uppercase fw-700 text-white mb-30">محمد ابو شماله</h3>
											<p class="trainer-body">الذهاب إلى مركز رياضي أو ما يعرف   الذهاب  والإياب يجب أن يكون مناسباً في المصاريف الشهرية و يتوفر به جميع الأدوات والمعدات المطلوبة و نظافة المكان و أن تشعر فيه بالراحة. أما إذا كنت ترغب في ممارسة الرياضة في منزلك وهذا ما يفعله المعظم و طبعاً مزاياه لا رسوم دخول ولاحشود ويمكنك الأنتقال من تمرين إلى آخر دون راحة والعيب هو أنه ليس لديك أحد كي يشجعك أو يوجهك الذهاب إلى مركز رياضي أو ما يعرف   الذهاب  والإياب يجب أن يكون مناسباً في المصاريف الشهرية و يتوفر به جميع الأدوات والمعدات المطلوبة و نظافة المكان و أن تشعر فيه بالراحة. أما إذا كنت ترغب في ممارسة الرياضة في منزلك وهذا ما يفعله المعظم و طبعاً مزاياه لا رسوم دخول ولاحشودز ويمكنك الأنتقال من تمرين إلى آخر دون راحة والعيب هو أنه ليس لديك أحد كي يشجعك أو يوجهك الذهاب إلى مركز رياضي أو ما يعرف   الذهاب  والإياب يجب أن يكون مناسباً في المصاريف الشهرية و يتوفر به جميع الأدوات والمعدات المطلوبة و نظافة المكان و أن تشعر فيه بالراحة. أما إذا كنت ترغب في ممارسة الرياضة في منزلك وهذا ما يفعله المعظم و طبعاً مزاياه لا رسوم دخول ولاحشود ويمكنك الأنتقال من تمرين إلى آخر دون راحة والعيب هو أنه ليس لديك أحد كي يشجعك أو يوجهك الذهاب إلى مركز رياضي أو ما يعرف   الذهاب  والإياب يجب أن يكون مناسباً في المصاريف الشهرية و يتوفر به جميع الأدوات والمعدات المطلوبة و نظافة المكان و أن تشعر فيه بالراحة. أما إذا كنت ترغب في ممارسة الرياضة في منزلك وهذا ما يفعله المعظم و طبعاً مزاياه لا رسوم دخول ولاحشود ويمكنك الأنتقال من تمرين إلى آخر دون راحة والعيب هو أنه ليس لديك أحد كي يشجعك أو يوجهك  </p>
											 <div class="buttons-group flex align-items-center mt-30">
												<a href="#" class="btn btn-bottom-border mr-10">Facebook</a>
												<a href="#" class="btn btn-bottom-border mr-10">Instagram</a>
												<a href="#" class="btn btn-bottom-border">Linkedin</a>
											</div>
										</div> <!-- end .col-12 col-lg-5 lg-push-1 -->

										<div class="col-12 col-md-6 col-lg-5  offset-lg-1">
											<img src="@/assets/img/about/layer-5.png" class="img-fluid trainer-img" alt="">
										</div>
									</div> <!-- end #trainer-bio -->
									<div class="row trainer-bio d-flex align-items-center">
										<div class="col-12 col-md-6 col-lg-6">
											<h3 class="text-white text-uppercase fw-700 text-white mb-30">محمد ابو شماله</h3>
											<p class="trainer-body">الذهاب إلى مركز رياضي أو ما يعرف   الذهاب  والإياب يجب أن يكون مناسباً في المصاريف الشهرية و يتوفر به جميع الأدوات والمعدات المطلوبة و نظافة المكان و أن تشعر فيه بالراحة. أما إذا كنت ترغب في ممارسة الرياضة في منزلك وهذا ما يفعله المعظم و طبعاً مزاياه لا رسوم دخول ولاحشود ويمكنك الأنتقال من تمرين إلى آخر دون راحة والعيب هو أنه ليس لديك أحد كي يشجعك أو يوجهك الذهاب إلى مركز رياضي أو ما يعرف   الذهاب  والإياب يجب أن يكون مناسباً في المصاريف الشهرية و يتوفر به جميع الأدوات والمعدات المطلوبة و نظافة المكان و أن تشعر فيه بالراحة. أما إذا كنت ترغب في ممارسة الرياضة في منزلك وهذا ما يفعله المعظم و طبعاً مزاياه لا رسوم دخول ولاحشودز ويمكنك الأنتقال من تمرين إلى آخر دون راحة والعيب هو أنه ليس لديك أحد كي يشجعك أو يوجهك الذهاب إلى مركز رياضي أو ما يعرف   الذهاب  والإياب يجب أن يكون مناسباً في المصاريف الشهرية و يتوفر به جميع الأدوات والمعدات المطلوبة و نظافة المكان و أن تشعر فيه بالراحة. أما إذا كنت ترغب في ممارسة الرياضة في منزلك وهذا ما يفعله المعظم و طبعاً مزاياه لا رسوم دخول ولاحشود ويمكنك الأنتقال من تمرين إلى آخر دون راحة والعيب هو أنه ليس لديك أحد كي يشجعك أو يوجهك الذهاب إلى مركز رياضي أو ما يعرف   الذهاب  والإياب يجب أن يكون مناسباً في المصاريف الشهرية و يتوفر به جميع الأدوات والمعدات المطلوبة و نظافة المكان و أن تشعر فيه بالراحة. أما إذا كنت ترغب في ممارسة الرياضة في منزلك وهذا ما يفعله المعظم و طبعاً مزاياه لا رسوم دخول ولاحشود ويمكنك الأنتقال من تمرين إلى آخر دون راحة والعيب هو أنه ليس لديك أحد كي يشجعك أو يوجهك  </p>
											 <div class="buttons-group flex align-items-center mt-30">
												<a href="#" class="btn btn-bottom-border mr-10">Facebook</a>
												<a href="#" class="btn btn-bottom-border mr-10">Instagram</a>
												<a href="#" class="btn btn-bottom-border">Linkedin</a>
											</div>
										</div> <!-- end .col-12 col-lg-5 lg-push-1 -->

										<div class="col-12 col-md-6 col-lg-5  offset-lg-1">
											<img src="@/assets/img/about/layer-5.png" class="img-fluid trainer-img" alt="">
										</div>
									</div> <!-- end #trainer-bio -->
								
								
							</VueSlickCarousel>
							 <div class="custom-arrow">
								  <button class="slick-prev slick-arrow gradient-bg gradient-hover"  @click="showPrev"><i class="ti-angle-left"></i></button> 
                           <button class="slick-next slick-arrow gradient-bg gradient-hover" @click="showNext"><i class="ti-angle-right"></i></button> 
							 </div>	
						  </div>
					   </template>
					<!-- end .trainer-slide -->
				</div> <!-- end .container -->
			</section> <!-- end #our-trainers -->			  
			<section id="testimonials" class="section-full p-relative paroller" style="background: #202126 !important;">
				<div class="container">
					<div class="row">
						<div class="col-12 col-lg-10 mx-auto">
							<VueSlickCarousel  v-bind="testimonial">							
								<article class="testimonial-item swiper-slide text-center">
									<figure class="author mb-40">
										<div class="inline-flex align-center">
											<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 19 30" style="enable-background:new 0 0 19 30;" xml:space="preserve">
												<g><g><path class="svg-fill-text" d="M14,0v30h5V0H14z M7,30h5V0H7V30z M0,25h5V5H0V25z"/></g></g>
											</svg>
											<img src="@/assets/img/author-1.jpg" class="ml-20 mr-20 mb-25" alt="">
											<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 19 30" style="enable-background:new 0 0 19 30;" xml:space="preserve">
												<g><g><path class="svg-fill-text" d="M0,30h5V0H0V30z M7,30h5V0H7V30z M14,5v20h5V5H14z"/></g></g>
											</svg>
										</div>
										<figcaption>
											<h4 class="fw-700 fs-18 text-uppercase text-white">Ryan Gosling</h4>
											<span class="d-block">Manager, Google Inc</span>
										</figcaption>
									</figure>
									<blockquote class="fs-18 text-italic text-white">الذهاب إلى مركز رياضي أو ما يعرف   الذهاب  والإياب يجب أن يكون مناسباً في المصاريف الشهرية و يتوفر به جميع الأدوات والمعدات المطلوبة و نظافة المكان و أن تشعر فيه بالراحة. أما إذا كنت ترغب في ممارسة الرياضة في منزلك وهذا ما يفعله المعظم و طبعاً مزاياه لا رسوم دخول ولاحشود ويمكنك الأنتقال من تمرين إلى آخر دون راحة والعيب هو أنه ليس لديك أحد كي يشجعك أو يوجهك الذهاب إلى مركز رياضي أو ما يعرف   الذهاب  والإياب يجب أن يكون مناسباً في المصاريف الشهرية و يتوفر به جميع الأدوات والمعدات المطلوبة و نظافة المكان و أن تشعر فيه بالراحة. أما إذا كنت ترغب في ممارسة الرياضة في منزلك وهذا ما يفعله المعظم و طبعاً مزاياه لا رسوم دخول ولاحشودز ويمكنك الأنتقال من تمرين إلى آخر دون راحة والعيب هو أنه ليس لديك أحد كي يشجعك أو يوجهك الذهاب إلى مركز رياضي أو ما يعرف   الذهاب  والإياب يجب أن يكون مناسباً في المصاريف الشهرية و يتوفر به جميع الأدوات والمعدات المطلوبة و نظافة المكان و أن تشعر فيه بالراحة. أما إذا كنت ترغب في ممارسة الرياضة في منزلك وهذا ما يفعله المعظم و طبعاً مزاياه لا رسوم دخول ولاحشود ويمكنك الأنتقال من تمرين إلى آخر دون راحة والعيب هو أنه ليس لديك أحد كي يشجعك أو يوجهك الذهاب إلى مركز رياضي أو ما يعرف   الذهاب  والإياب يجب أن يكون مناسباً في المصاريف الشهرية و يتوفر به جميع الأدوات والمعدات المطلوبة و نظافة المكان و أن تشعر فيه بالراحة. أما إذا كنت ترغب في ممارسة الرياضة في منزلك وهذا ما يفعله المعظم و طبعاً مزاياه لا رسوم دخول ولاحشود ويمكنك الأنتقال من تمرين إلى آخر دون راحة والعيب هو أنه ليس لديك أحد كي يشجعك أو يوجهك  </blockquote>
								</article>
								<article class="testimonial-item swiper-slide text-center">
									<figure class="author mb-40">
										<div class="inline-flex align-center">
											<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 19 30" style="enable-background:new 0 0 19 30;" xml:space="preserve">
												<g><g><path class="svg-fill-text" d="M14,0v30h5V0H14z M7,30h5V0H7V30z M0,25h5V5H0V25z"/></g></g>
											</svg>
											<img src="@/assets/img/author-1.jpg" class="ml-20 mr-20 mb-25" alt="">
											<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 19 30" style="enable-background:new 0 0 19 30;" xml:space="preserve">
												<g><g><path class="svg-fill-text" d="M0,30h5V0H0V30z M7,30h5V0H7V30z M14,5v20h5V5H14z"/></g></g>
											</svg>
										</div>
										<figcaption>
											<h4 class="fw-700 fs-18 text-uppercase text-white">James Bond</h4>
											<span class="d-block">Manager, Google Inc</span>
										</figcaption>
									</figure>
									<blockquote class="fs-18 text-italic text-white">“Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.”</blockquote>
								</article>
								<article class="testimonial-item swiper-slide text-center">
									<figure class="author mb-40">
										<div class="inline-flex align-center">
											<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 19 30" style="enable-background:new 0 0 19 30;" xml:space="preserve">
												<g><g><path class="svg-fill-text" d="M14,0v30h5V0H14z M7,30h5V0H7V30z M0,25h5V5H0V25z"/></g></g>
											</svg>
											<img src="@/assets/img/author-1.jpg" class="ml-20 mr-20 mb-25" alt="">
											<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 19 30" style="enable-background:new 0 0 19 30;" xml:space="preserve">
												<g><g><path class="svg-fill-text" d="M0,30h5V0H0V30z M7,30h5V0H7V30z M14,5v20h5V5H14z"/></g></g>
											</svg>
										</div>
										<figcaption>
											<h4 class="fw-700 fs-18 text-uppercase text-white">Travor James</h4>
											<span class="d-block">Manager, Google Inc</span>
										</figcaption>
									</figure>
									<blockquote class="fs-18 text-italic text-white">“Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur.”</blockquote>
								</article>
							</VueSlickCarousel><!-- end .testimonials -->
						</div> <!-- end .col-12.col-md-4 -->
					</div> <!-- end .row -->
				</div> <!-- end .container -->
			</section> <!-- end #testimonials -->

		</main> <!-- end .site-main -->
</template>

<script>
import VueSlickCarousel from 'vue-slick-carousel'
import 'vue-slick-carousel/dist/vue-slick-carousel.css' 
// import 'vue-slick-carousel/dist/vue-slick-carousel-theme.css'

export default {
  components: { VueSlickCarousel },	
  name: 'About',
 data() {
    return {       
        trainerslider: {         
		 arrows: false,
		 dots: false ,
		 

        },
        testimonial: {
          arrows: false,

        },
      componentKey: 0,
     
    };
  },
  methods: {
    forceRerender() {
      this.componentKey += 1;  
    },
    showNext() {
        this.$refs.carousel.next()
      },
    showPrev() {
        this.$refs.carousel.prev()
      },  
  }
}
</script>
<style  scoped>
.trainer-slider{
	position: relative;
}
.trainer-body{
	color: #fff;
}
.custom-arrow{
	position :absolute;
	right: 41.6%;
	bottom: 0;
}
.trainer-slider .custom-arrow .slick-prev {
    left: -webkit-calc(50% + 15px);
    left: -moz-calc(50% + 15px);
    left: calc(50% + 15px);
    bottom: 70px;
}
.trainer-img{
	max-height: 100%;
	background-size: cover;
	height:600px;
}
</style>

